...
saturation = parse_expr('(T2X + R2X + 2*T2X2 + 2*R2X2)/2/(T2 + R2 + T2X + R2X + T2X2 + R2X2)')
...
Y = parse_expr("(K*X/KT*(1+X/KT)+X/KR*(1+X/KR))/(K*(1+X/KT)**2+(1+X/KR)**2)"). \
                   subs("K", parse_expr("k_1/k1")). \
                   subs("KT", parse_expr("k_2/k2")). \
                   subs("KR", parse_expr("k_3/k3")).simplify()
